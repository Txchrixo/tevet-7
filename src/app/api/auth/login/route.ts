 ...
 * relative `/api/auth/login` (same origin, always reachable), and Next.js
 * forwards the request server-side to
 * `http://localhost:8001/api/auth/login`. Works on any port the Preview
 * Panel uses — no Caddy gateway or `XTransformPort` query needed.
 *
 * The Authorization header is forwarded if present (rare for login, but
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const backendRes = await fetch(`${BACKEND_ORIGIN}/api/auth/login`, {
      method: "POST",
      headers,
      body,
