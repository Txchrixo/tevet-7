...
start_mini_services() {
        ...
        for service_dir in "$mini_services_dir"/*; do
                ...
                # setsid: detach into its own session so the service survives dev.sh
                # exit (without it, SIGHUP kills the service when the script returns).
                setsid bash -c "
                        cd '$service_dir'
                        ...
                        exec bun run dev
                " >"$PROJECT_DIR/.zscripts/mini-service-${service_name}.log" 2>&1 < /dev/null &

                local service_pid=$!
                echo "[$service_name] Started in background (PID: $service_pid, session detached via setsid)"
